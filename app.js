...
function renderRoutines() {
  const cards = state.routines.map(routine => {
    ...
    const lastText = recent?.completedAt ? `Last completed ${formatDate(recent.completedAt)}` : 'No completed sessions';
    return `
  ...
  app.innerHTML = `
    <main class="screen">
      <header class="topbar"><div class="suite-heading"><p>Bloody Dave's Suite</p><h1><span>Lift</span> Log</h1><nav aria-label="Bloody Dave's Suite"><a href="https://bloody-recipes.pages.dev">Recipes</a><a href="https://fragments.hope-johnstone.com">Fragments</a><a href="https://bloody-daves-quiet-timer.pages.dev">Quiet Timer</a><a href="https://bloody-daves-get-list.pages.dev">Get List</a><a class="current" href="#">Lift Log</a></nav></div>${state.activeWorkout ? '<button class="btn btn-primary" data-action="resume-workout">Resume</button>' : ''}</header>
      <div class="content">
        <div class="toolbar">
        ...
        ${cards || `
          <section class="empty">
            ...
            <p class="muted">Create a routine from the exercise library.</p>
...
function renderWorkoutExercise(exercise, nextSetId, labelPrefix = '') {
  return `
    <section class="workout-exercise card ${labelPrefix ? 'inside-superset' : ''}">
      ...
      <div class="workout-set-grid workout-set-head"><span>Set</span><span>Previous</span><span>kg</span><span>Reps</span><span>Done</span></div>
      ${exercise.sets.map((set, setIndex) => `
        <div class="workout-set-grid ${set.isCompleted ? 'completed' : ''} ${set.id === nextSetId ? 'next-set' : ''}" data-set-id="${set.id}">
          ...
          <button class="complete-btn ${set.isCompleted ? 'done' : ''}" aria-label="Mark ${exercise.exerciseName} set ${setIndex + 1} ${set.isCompleted ? 'incomplete' : 'complete'}" data-workout-complete="${set.id}">${set.isCompleted ? '✓' : '○'}</button>
